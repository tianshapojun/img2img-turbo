...
if __name__ == "__main__":
    ...
    parser.add_argument('--input_img_dir', type=str, required=True, help='path to the input image')
    parser.add_argument('--prompt', type=str, required=True, help='the prompt to be used')
    parser.add_argument('--model_name', type=str, default='', help='name of the pretrained model to be used')
    parser.add_argument('--model_path', type=str, default='', help='path to a model state dict to be used')
    parser.add_argument('--output_dir', type=str, default='output', help='the directory to save the output')
    parser.add_argument('--output_name', type=str, default=None, help='the name of the output image')
    ...
    if '-' in args.input_img_dir:
        img_list = args.input_img_dir.split('-')
    else:
        items = os.listdir(args.input_img_dir)
        img_list = [os.path.join(args.input_img_dir, item) for item in items 
                if (item.endswith(".jpg") or item.endswith(".png")) and os.path.isfile(os.path.join(args.input_img_dir, item))
                #and item.startswith("2000")
                ]
    img_list.sort()
    ...
    for img_path in tqdm(img_list):
        ...
        with torch.no_grad():
            ...
            output_pil = transforms.ToPILImage()(output_image[0,:,:H//2,:].cpu() * 0.5 + 0.5)

        # save the output image
        if args.output_name is not None:
            output_pil.save(os.path.join(args.output_dir, args.output_name))
        else:
